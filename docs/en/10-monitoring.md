...
> The ServiceMonitor is generated by Helm values (`controller.metrics.serviceMonitor.enabled: true`), so do not add a separate ServiceMonitor manifest in overlays.
